...
public inline fun <reified T: VertexConsumer> T.pos2d(pos: Vec2d): T = this.vertex(pos.x, pos.y, 0.0) as T
/** Sets the position with Z = 0 */
public inline fun <reified T: VertexConsumer> T.pos2d(x: Number, y: Number): T = this.vertex(x.toDouble(), y.toDouble(), 0.0) as T

/** Sets the position with Z = 0 */
public inline fun <reified T: VertexConsumer> T.pos2d(matrix: Matrix3d, pos: Vec2d): T = this.vertex(
    matrix.transformX(pos.x, pos.y),
    matrix.transformY(pos.x, pos.y),
    0.0
) as T
/** Sets the position with Z = 0 */
public inline fun <reified T: VertexConsumer> T.pos2d(matrix: Matrix3d, x: Number, y: Number): T = this.vertex(
