...
internal object DrawingUtil {
    fun draw(sprite: ISprite, vb: VertexConsumer, matrix: Matrix3d, x: Float, y: Float, width: Float, height: Float, animFrames: Int, tint: Color) {
    ...
    private fun drawSimple(sprite: ISprite, vb: VertexConsumer, matrix: Matrix3d, x: Float, y: Float, width: Float, height: Float, animFrames: Int, tint: Color) {
        val minX = x
        val minY = y
        val maxX = x + width
        val maxY = y + height
        ...
        vb.pos2d(matrix, minX, maxY).color(tint).texture(minU, maxV).next()
        vb.pos2d(matrix, maxX, maxY).color(tint).texture(maxU, maxV).next()
        vb.pos2d(matrix, maxX, minY).color(tint).texture(maxU, minV).next()
        vb.pos2d(matrix, minX, minY).color(tint).texture(minU, minV).next()
    }

    private fun drawComplex(sprite: ISprite, vb: VertexConsumer, matrix: Matrix3d, x: Float, y: Float, width: Float, height: Float, animFrames: Int, tint: Color) {
        ...
        xSections.forEach { xSection ->
            ySections.forEach { ySection ->
                ...
                vb.pos2d(matrix, minX, maxY).color(tint).texture(u0.toFloat(), v0.toFloat()).next()
                vb.pos2d(matrix, maxX, maxY).color(tint).texture(u1.toFloat(), v1.toFloat()).next()
                vb.pos2d(matrix, maxX, minY).color(tint).texture(u2.toFloat(), v2.toFloat()).next()
                vb.pos2d(matrix, minX, minY).color(tint).texture(u3.toFloat(), v3.toFloat()).next()
            }
        }
    }
